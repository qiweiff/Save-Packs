...
namespace 存背包
{
    ...
    public class 存背包 : TerrariaPlugin
    {
        ...
        /// 插件版本
        public override Version Version => new(1, 4, 0, 0);
        /// 插件处理
        ...
        private void OnInitialize(EventArgs args)//游戏初始化的狗子
        {
            ...
            Commands.ChatCommands.Add(new Command("存背包.全部", 指令4, "保存全部") { });
            Commands.ChatCommands.Add(new Command("存背包.重载", 重载, "reload") { });

        }
        public class FoundPlayer//PlayerManager里的方法
        {
            // id从1开始，-1表示非ssc玩家
            // 此id，仅对db数据才有效
            public int ID = -1;
        ...
        private void 指令1(CommandArgs args)
        {
            try
            {
                if (args.Parameters.Count == 0)
                {
                    ...
                    args.Player.SendInfoMessage($"正确指令：/保存全部");
                    args.Player.SendInfoMessage($"正确指令：/读取背包 背包ID");
                    args.Player.SendInfoMessage($"正确指令：/读取背包 <返回/确定>");
                    args.Player.SendInfoMessage($"正确指令：/删除背包 背包ID");
                ...
                FoundPlayer found = GetPlayer(args.Player, args.Parameters[0]);
                if (!found.valid)
                {
                    return;
                }
                if (found.online)
                {
                    List<数据> pack = SavePack(found.plr);
                    配置.保存的背包.Add(new() { });
                ...
                {
                    List<数据> pack = SavePacksOffLine(found.ID);
                    配置.保存的背包.Add(new() { });
                    int L = 配置.保存的背包.Count - 1;
                    配置.保存的背包[L] = pack[0];
                    配置.保存的背包[L].背包ID = L;
                    ...
                    args.Player.SendInfoMessage($"[存背包]保存离线玩家[c/FF3333:{args.Parameters[0]}]的背包成功！");
                    File.WriteAllText(path, JsonConvert.SerializeObject(配置, Formatting.Indented));
                }
                return;
        ...
        private void 指令3(CommandArgs args)
        {
            try
            {
                if (args.Parameters.Count == 0)
                {
                    args.Player.SendInfoMessage($"正确指令：/保存背包 玩家名 <备注>");
                    args.Player.SendInfoMessage($"正确指令：/保存全部");
                    args.Player.SendInfoMessage($"正确指令：/读取背包 背包");
                    args.Player.SendInfoMessage($"正确指令：/读取背包 <返回/确定>");
                    args.Player.SendInfoMessage($"正确指令：/删除背包 背包ID");
                    args.Player.SendInfoMessage($"背包列表：\n");
                    foreach (var z in 配置.保存的背包)
        ...
        private void 指令4(CommandArgs args)
        {
            try
            {
                var allaccount = TShock.UserAccounts.GetUserAccounts();
                for (var a = 1; a < allaccount.Count; a++)
                {
                    if (allaccount[a] == null) { break; }
                    var n = allaccount[a].Name;
                    FoundPlayer found = GetPlayer(args.Player, n);
                    if (!found.valid)
                        break;
                    if (found.online)
                    {
                        ...
                        配置.保存的背包[L] = pack[0];
                        配置.保存的背包[L].背包ID = L;
                        配置.保存的背包[L].背包备注 = DateTime.Now.ToString() + ">>>" + n;
                        args.Player.SendInfoMessage($"[存背包]保存[c/FF3333:{n}]的背包成功！");
                        File.WriteAllText(path, JsonConvert.SerializeObject(配置, Formatting.Indented));
                    }
                    else
                    {
                        ...
                        配置.保存的背包[L] = pack[0];
                        配置.保存的背包[L].背包ID = L;
                        配置.保存的背包[L].背包备注 = DateTime.Now.ToString() + " >>>玩家名：" + n;
                        args.Player.SendInfoMessage($"[存背包]保存离线玩家[c/FF3333:{n}]的背包成功！");
                        File.WriteAllText(path, JsonConvert.SerializeObject(配置, Formatting.Indented));
                    }
                }
                args.Player.SendInfoMessage($"[存背包]保存全部玩家的背包成功！");
            }
            catch { args.Player.SendErrorMessage($"[存背包]保存全部背包失败！"); }
        }
        ...
        static List<数据> SavePacksOffLine(int userid)
        {
            List<Inventory> 背包 = new() { };
            List<数据> 增益 = new() { };
            //var name = username;
            ...
            if (data != null)
            {
                ...
                for (int i = 0; i < NetItem.MaxInventory; i++)
                {
                    if (i < 350)
                    {
                        var netID = data.inventory[i].NetId;
                        var stack = data.inventory[i].Stack;
                        var prefix = data.inventory[i].PrefixId;
                        背包.Add(new()
                        {
                            格子 = i,
                            物品 = netID,
                ...
                var statLifeMax = data.maxHealth;
                var statManaMax = data.maxMana;
                var anglerQuestsFinished = data.questsCompleted;
                var extraAccessory = zh(data.extraSlot);
                var aUsingBiomeTorches = zh(data.unlockedBiomeTorches);
                var ateArtisanBread = zh(data.ateArtisanBread);
                var usedAegisCrystal = zh(data.usedAegisCrystal);
                var usedAegisFruit = zh(data.usedAegisFruit);
                var usedArcaneCrystal = zh(data.usedArcaneCrystal);
                var usedGalaxyPearl = zh(data.usedGalaxyPearl);
                var usedGummyWorm = zh(data.usedGummyWorm);
                var usedAmbrosia = zh(data.usedAmbrosia);
                var unlockedSuperCart = zh(data.unlockedSuperCart);
                增益.Add(new()
                {
                    背包 = 背包,
                    血量 = statLifeMax,
